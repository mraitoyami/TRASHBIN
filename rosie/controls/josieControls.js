...
class ThirdPersonCameraController {
  ...
  setupMouseControls() {
    ...
    if ('ontouchstart' in window || navigator.maxTouchPoints > 0) {
      ...
      const isTouchOverUI = (touch) => {
        const element = document.elementFromPoint(touch.clientX, touch.clientY);
        if (!element) return false;
        
        // Check for specific IDs or parents
        return (
      ...
      this.domElement.addEventListener('touchstart', (e) => {
        ...
        // Find a touch that is NOT over UI
        for (let i = 0; i < e.changedTouches.length; i++) {
          const touch = e.changedTouches[i];
          if (!isTouchOverUI(touch)) {
            activeTouchId = touch.identifier;
            lastTouchPos = { x: touch.clientX, y: touch.clientY };
            // Don't preventDefault here to allow clicks to pass through if needed, 
            // though we are on the canvas anyway.
            break;
          }
        }
      }, { passive: false });
      ...
      this.domElement.addEventListener('touchmove', (e) => {
        if (!this.enabled || activeTouchId === null) return;
        ...
        for (let i = 0; i < e.changedTouches.length; i++) {
          ...
          if (touch.identifier === activeTouchId) {
            ...
            lastTouchPos = { x: touch.clientX, y: touch.clientY };
            e.preventDefault(); // Prevent scrolling while looking around
            break;
          }
        }
      }, { passive: false });
